...
## Modo Multi-Página (App Completa)

Para proyectos que requieren múltiples episodios navegables como una app cohesiva, consultar el archivo de referencia completo:

> **→ Leer `references/multi-page-app.md`** cuando el usuario pida:
> - App/sitio con múltiples episodios enlazados
> - Navegación entre páginas con estética consistente
> - JS modular (módulos .js separados importados desde HTML)
> - Página índice/archivo de episodios
> - Cualquier estructura con más de un archivo HTML

El archivo cubre: arquitectura de carpetas, sistema de navegación, módulos JS compartidos, contratos entre archivos, y patrones para mantener coherencia visual cross-page.

## Assets / Templates

En `assets/templates/` hay archivos base listos para copiar:
- `episode.html` — template de episodio individual (todos los componentes del skill ya incluidos)
- `index.html` — template de página índice/galería
- `shared.js` — módulo JS con funciones reutilizables entre páginas (boot sequence, observer, glitch utils)
- `nav.js` — navegación prev/next dinámica entre episodios (requiere `episodes.js`)
- `episodes.js` — registro central de episodios (fuente de verdad para metadata)

> **→ Usar estos templates como punto de partida** en lugar de construir desde cero. Reducen errores de consistencia visual.

---

## Workflow: Generar App Multi-Página Completa

Cuando el usuario pida la app entera (o varios archivos entregables a la vez), seguir este orden:

### Paso 1 — Estructura de archivos a generar
```
esquizoai-app/
├── index.html          ← desde assets/templates/index.html
├── ep01.html           ← desde assets/templates/episode.html (personalizar)
├── ep02.html           ← idem
├── ep03.html           ← idem
└── js/
    ├── shared.js       ← desde assets/templates/shared.js (copiar sin cambios)
    ├── episodes.js     ← desde assets/templates/episodes.js (actualizar episodios si hay nuevos)
    └── nav.js          ← desde assets/templates/nav.js (copiar sin cambios)
```

### Paso 2 — Orden de generación
1. `js/episodes.js` primero — es la fuente de verdad que todo lo demás referencia
2. `js/shared.js` y `js/nav.js` — copiar desde templates, sin modificaciones
3. `index.html` — galería de episodios (importa `episodes.js` para renderizar cards)
4. `epNN.html` — uno por episodio, personalizando: título, subtítulo, boot lines, bloques de contenido, y EP_ID en el script final

### Paso 3 — Checklist por episodio
Al generar cada `epNN.html` verificar:
- [ ] `EP_ID` correcto en el `<script type="module">` final
- [ ] Boot lines temáticas para ese episodio (no reutilizar las del template)
- [ ] Bloque `.ep-anterior` presente si es ep02 o posterior
- [ ] `damage_definition.json: FILE NOT FOUND` en alguna boot line
- [ ] CSS de `.ep-nav` incluido en el `<style>` (copiar desde comentario en `nav.js`)
- [ ] `<div id="ep-nav"></div>` presente antes de cerrar `.wrap`

### Paso 4 — Advertencia de servidor local
Al entregar archivos con módulos ES, avisar siempre:
> Los módulos JS (`type="module"`) requieren servidor local para funcionar. Abrir con `python -m http.server 8080` o Live Server de VSCode. No funcionan con `file://` directo.

---

## Cuándo Usar Templates vs Construir Desde Cero

| Situación | Approach |
|---|---|
| Episodio nuevo para app existente | Copiar `episode.html`, cambiar EP_ID + contenido |
| App completa desde cero | Copiar todos los templates, personalizar `episodes.js` |
| Artefacto único HTML autocontenido | Construir inline (modo original del skill), ignorar templates de multi-página |
| El usuario pide "un HTML" sin más contexto | Modo artefacto único — no usar módulos externos |
| El usuario menciona "app", "sitio", "navegar entre episodios" | Modo multi-página — usar templates + módulos |
